...
void main(uint3 dispatchID : SV_DispatchThreadID)
{
    ...
    uint2 coords = (uint2(resolution, resolution) - pixel_coord) % (resolution - 1);
    ...
    float2 hX = -multiplyByI(h * (wave_vector.x / length(wave_vector))) * ocean_choppiness;
    float2 hZ = -multiplyByI(h * (wave_vector.y / length(wave_vector))) * ocean_choppiness;
