...
void cs_main(uint3 dispatchID : SV_DispatchThreadID)
{
    ...
    float U10 = length(float2(wind_direction_x, wind_direction_y));
    ...
    float cosPhi = dot(normalize(float2(wind_direction_x, wind_direction_y)), normalize(wave_vector));
	
    float S = (1.0 / (2.0 * PI)) * pow(k, -4.0) * (Bl + Bh) * (1.0 + Delta * (2.0 * cosPhi * cosPhi - 1.0));
	
    float dk = 2.0 * PI / ocean_size;
