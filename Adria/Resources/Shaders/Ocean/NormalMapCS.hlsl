...
void main(uint3 DTid : SV_DispatchThreadID)
{
    ...
    float texel_size = ocean_size * texel;
    
   
    float3 _right    = DisplacementMap.Load(uint3(uint2(clamp(pixel_coord.x + 1, 0, resolution - 1), pixel_coord.y), 0)).xyz;
    float3 _left     = DisplacementMap.Load(uint3(uint2(clamp(pixel_coord.x - 1, 0, resolution - 1), pixel_coord.y), 0)).xyz;
    float3 _top      = DisplacementMap.Load(uint3(uint2(pixel_coord.x, clamp(pixel_coord.y - 1, 0, resolution - 1)), 0)).xyz;
    float3 _bottom   = DisplacementMap.Load(uint3(uint2(pixel_coord.x, clamp(pixel_coord.y + 1, 0, resolution - 1)), 0)).xyz;
