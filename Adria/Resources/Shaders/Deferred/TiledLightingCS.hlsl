          ...
          uint  groupIndex : SV_GroupIndex)
{
    ...
    float min_z_pixel = camera_far;
    float max_z_pixel = camera_near;
    ...
    bool validPixel = view_depth >= camera_near && view_depth < camera_far;
    ...
    float3 Lo = 0.0f;
    if (all(dispatchThreadId.xy < screen_resolution))
    {
        for (int i = 0; i < tile_num_lights; ++i)
        {
            StructuredLight structured_light = lights[tile_light_indices[i]];
            Light light = CreateLightFromStructured(structured_light);
    ...
    if(visualize_tiled)
    {
        ...
        float l = saturate((float) tile_num_lights / lights_count_white) * heap_map_max;
