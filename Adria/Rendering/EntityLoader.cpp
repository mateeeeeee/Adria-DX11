        ...
        for (size_t i = 0; i < scene.nodes.size(); ++i)
        {
            auto LoadNode = [&](int node_index) {
				...
				for (size_t i = 0; i < node_mesh.primitives.size(); ++i)
				{
					...
					XMMATRIX model = transform * params.model_matrix;
			...
			if (node.mesh < 0 || node.mesh >= model.meshes.size())
			{
				...
				{
					for (int node_index : node.children) LoadNode(node_index);
				}
			}
			else LoadNode(scene.nodes[i]);
        }
